...
def test_game_create_rate_limit(mock_rate_limiter_time):
    payload = {"player_id": "p1", "player_name": "PlayerOne"}
    
    # --- T = 0 ---
    # Request 1: Should pass (1/sec limit)
    try:
    ...
    # Request 2: Should fail immediately (still T=0)
    res = client.post("/games", json=payload)
    assert res.status_code == 429
    assert "Rate limit exceeded" in res.json()["detail"]
    
    # --- T = 1.1s (Advance time) ---
    # The bucket refills at 1 token/sec. After 1.1s, we should have 1.1 tokens (capped at 1.0).
    mock_rate_limiter_time.return_value = MOCKED_START_TIME + 1.1
    
    # Request 3: Should pass now
    # Note: We rely on the fact that the rate limiter calculates refill based on current time
    try:
        res = client.post("/games", json=payload)
        assert res.status_code != 429
    except Exception:
...
def test_query_rate_limit(mock_rate_limiter_time):
    ...
    # 11th Request: Fail (Empty bucket)
    res = client.get(f"/games/{game_id}")
    assert res.status_code == 429
